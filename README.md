...
- 本机缓存按账号隔离；正常退出登录时只清除当前账号的本机缓存，不删除云端数据。
- 登录身份冲突或无法确认时，可一键切断本设备与职序的登录状态并清除职序缓存，不删除任何云端求职数据。
...
- 当前发布版本与部署包不包含线上用户数据、备份文件、部署密钥或真实管理员 ID。
...
- 将数据库结构变更收敛到发布迁移，修复冷启动时登录读取被结构检查阻塞的问题，并补充请求边界回归测试。
...
故障现象、影响范围、原因、修复边界和验证证据另行保存在 [CHANGELOG.md](./CHANGELOG.md)，便于按日期回溯，不在公开文档中记录任何真实用户数据或认证凭证。
...
| 2026-09-01 15:37 | 修复智能助手数据版本一致性、完整查询、字段解析与质量统计口径；以匿名 9 公司、21 岗位真实形状回归验证 |
| 2026-09-01 16:12 | 完善登录状态保护：支持一键切断本设备登录、清除职序账号缓存并重新选择登录方式，优化手机端排版与异常恢复提示 |
| 2026-09-07 02:17 | 修复登录读取超时：数据库结构只在发布阶段迁移；15 个日常请求不再执行建表或兼容检查；补充 GitHub 会话快速返回与请求边界测试 |
...
当前 107 项自动化测试覆盖用户范围、云端状态、GitHub 登录、异常状态切断、无会话快速返回、请求期迁移边界、账号缓存识别、Agent 调用、额度并发、受控 CRUD、二次确认、重复执行保护、版本冲突、管理指标、相对日期，以及匿名 9 公司、21 岗位真实形状回归等关键路径。
...
db/schema.ts                 D1 结构测试常量
drizzle/                     生产数据库迁移（发布阶段执行）
public/                      PWA 图标、品牌资源与 Service Worker
```

## 公开仓库边界

本仓库公开用于产品作品展示、技术交流和代码审阅。公开源码不等于公开线上用户数据：实际账号、岗位记录、对话正文、备份和部署 Secret 均不在当前发布版本与部署包中。
